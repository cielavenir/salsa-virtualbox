...
# else
#  define SUPHNTIMP_ERROR(a_fReportErrors, a_id, a_FatalErrorMsgArgs) \
    do { \
        if (a_fReportErrors) supR3HardenedFatalMsg a_FatalErrorMsgArgs; \
        else { *(char *)(uintptr_t)(a_id) += 1; __debugbreak(); } \
    } while (0)
# endif
#else
# define SUPHNTIMP_ERROR(a_fReportErrors, a_id, a_szWhere, a_enmOp, a_rc, ...) \
